use catalog::{
    client::{artist::ArtistServiceProxy, release::ReleaseServiceProxy},
    repository::{artist::ArtistFilter, release::ReleaseFilter},
};
...
async fn test_get_release() {
    ...
    let client = ReleaseServiceProxy::new(&connection)
        .await
        .expect("unable to create client");
    ...
    for _ in 0..50 {
        ...
        let result = client.list_releases(filter).await;
